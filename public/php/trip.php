...
if($_SERVER['REQUEST_METHOD']=="POST")
{
    ...
    try {
          ...
          {
            $query = "INSERT INTO trip (tripid,userid,tripname,startdate,enddate,type,source,destination) VALUES (?,?,?,?,?,?,?,?)";
            $stmt = $pdo->prepare($query);
            $stmt->execute([$tripid,$userid,$tripname,$startdate,$enddate,$type,$source,$destination]);
            echo '<script>
